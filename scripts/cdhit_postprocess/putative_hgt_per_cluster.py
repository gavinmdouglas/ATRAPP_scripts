...
# Lightly modified version of code originally distributed with our other manuscript code previously:
...
formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    with gzip.open(args.geneinfo, "rb") as gene_fh:
        ...
        for gene_line in gene_fh:
            ...
            gene_id = gene_line[gene_tab_col['gene_id']]

            gene_to_genome[gene_id] = gene_line[gene_tab_col['genome_id']]

            if int(gene_line[gene_tab_col['scaffold_length']]) >= 5000:
                gene_scaffold_5000.add(gene_id)

            if int(gene_line[gene_tab_col['gene_length']]) >= 500:
                gene_length_500.add(gene_id)

    with open(args.output, 'w') as out_fh:
        print("\t".join(['gene1', 'gene2', 'gene1_genome', 'gene2_genome', 'highest_tax_diff', 'identity', 'genes_atleast_500bp', 'gene_scaffolds_atleast_5000bp']), file=out_fh)
            ...
            for filepath in input_fh:
                ...
                for seq_id in seq_ids:
                    ...
                    if seq_id == reverse_best_match and best_match not in ambig_seqs and best_match not in parsed_seqs:
                        ...
                        if tax[gene1_genome]['Genus'] == tax[gene2_genome]['Genus']:
                            continue
                        ...
                        if seq_id in gene_length_500 and best_match in gene_length_500:
                            gene_length_500_flag = 'Pass'
                        else:
                            gene_length_500_flag = 'Fail'

                        if seq_id in gene_scaffold_5000 and best_match in gene_scaffold_5000:
                            gene_scaffold_5000_flag = 'Pass'
                        else:
                            gene_scaffold_5000_flag = 'Fail'

                        print("\t".join([seq_id, best_match, gene1_genome, gene2_genome, highest_tax_diff, str(top_identities[seq_id]), gene_length_500_flag, gene_scaffold_5000_flag]),
                              file=out_fh)
